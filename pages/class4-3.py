...
col_num = st.number_input("請輸入欄位數", min_value=1, max_value=5, value=4, step=1)
...
with col1:
    selected_product = st.selectbox("請選擇商品", st.session_state.products.keys())
with col2:
    add_num = st.number_input(
        "請輸入庫存數量", min_value=1, max_value=10000, value=10, step=1
    )
if st.button("新增庫存"):
